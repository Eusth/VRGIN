  ...
  public class PinchDetector : Detector {
    ...
    protected virtual void Start() {
      ...
      if (GetComponent<IHandModel>() != null) {
                VRGIN.Core.VRLog.Warn("LeapPinchDetector should not be attached to the IHandModel's transform. It should be attached to its own transform.");
      }
      if (_handModel == null) {
                VRGIN.Core.VRLog.Warn("The HandModel field of LeapPinchDetector was unassigned and the detector has been disabled.");
        enabled = false;
      }
    }
