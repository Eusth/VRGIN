        ...
        if (CanDuplicate) {
          for (int i = 0; i < modelsCheckedOut.Count; i++) {
            if (modelsCheckedOut[i].HandModelType == modelType
              ...
              IHandModel spawnedModel = GameObject.Instantiate(modelToSpawn) as IHandModel;
    ...
    void Start() {
      foreach (ModelGroup collectionGroup in ModelPool) {
        ...
        if (collectionGroup.IsLeftToBeSpawned) {
          ...
          GameObject spawnedGO = GameObject.Instantiate(modelToSpawn.gameObject) as GameObject;
