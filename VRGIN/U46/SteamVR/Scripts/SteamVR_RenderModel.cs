...
public class SteamVR_RenderModel : MonoBehaviour
{
		...
		public CVRRenderModels instance
		{
			get
			{
				if (_instance == null && !failedLoadInterface)
				{
					...
					if (_instance == null)
					{
						Console.WriteLine("Failed to load IVRRenderModels interface version " + OpenVR.IVRRenderModels_Version);
	...
	public void UpdateModel()
	{
		...
		if (capacity <= 1)
		{
			Console.WriteLine("Failed to get render model name for tracked object " + index);
			return;
		}
	...
	private bool SetModel(string renderModelName)
	{
		...
		using (var holder = new RenderModelInterfaceHolder())
		{
			if (createComponents)
			{
				...
				Console.WriteLine("[" + gameObject.name + "] Render model does not support components, falling back to single mesh.");
			}
			...
			if (!string.IsNullOrEmpty(renderModelName))
			{
				...
				if (model == null || model.mesh == null)
				{
					...
					if (verbose)
						Console.WriteLine("Loading render model " + renderModelName);
	...
	private bool LoadComponents(RenderModelInterfaceHolder holder, string renderModelName)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				if (verbose)
					Console.WriteLine("Loading render model " + componentRenderModelName);
	...
	void OnEnable()
	{
		...
		{
			Console.WriteLine("Model override is really only meant to be used in the scene view for lining things up; using it at runtime is discouraged.  Use tracked device index instead to ensure the correct model is displayed for all users.");
			enabled = false;
			return;
		}
