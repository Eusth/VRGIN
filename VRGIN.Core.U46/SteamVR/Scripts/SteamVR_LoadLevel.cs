...
public class SteamVR_LoadLevel : MonoBehaviour
{
	...
	IEnumerator LoadLevel()
	{
		...
		if (loadExternalApp)
		{
			Console.WriteLine("Launching external application...");
			var applications = OpenVR.Applications;
			if (applications == null)
			{
				Console.WriteLine("Failed to get OpenVR.Applications interface!");
			}
			else
			{
				var workingDirectory = Directory.GetCurrentDirectory();
				Console.WriteLine("working directory: " + workingDirectory);
				Console.WriteLine("path: " + externalAppPath);
				Console.WriteLine("args: " + externalAppArgs);
				var error = applications.LaunchInternalProcess(externalAppPath, externalAppArgs, workingDirectory);
				Console.WriteLine("LaunchInternalProcessError: " + error);
				Application.Quit();
			}
		}
		else
		{
#if !(UNITY_5_2 || UNITY_5_1 || UNITY_5_0 || UNITY_4_5 || UNITY_4_5)
